...
import { Calendar, ClipboardList, Package, Sparkles, Truck, X } from "lucide-react";
import moment from "moment";
import { callGeminiAPI } from "@/lib/geminiApi";
import { createQc } from "@/lib/actions/qcAction";
import { QcSubmissionData, qcSubmissionSchema } from "@/lib/validation/qcValidationSchema";
...
const groupQcResultsByParameter = (results: any[]) => {
  if (!results || results.length === 0) {
    return [];
  }

  const grouped = results.reduce((acc, current) => {
    const { parameterId, parameter, resultKey, value } = current;
    ...
    if (resultKey === "main_value") {
      acc[parameterId].mainValue = value;
    } else {
      acc[parameterId].settings[resultKey] = value;
    }
...
function QcReview({ relatedData }: { relatedData: any }) {
  ...
  const handleAnalyzeQc = async (materialIndex: number) => {
    const materialQcData = relatedData.arrivalItems[materialIndex].material.name;
    const resultsText = relatedData.qcParameters.map((param: any, idx: number) => `${param.name}:${getValues(`materials.${materialIndex}.qcResults.${idx}.value`)}`).join(", ");
    setAnalyzingIndex(materialIndex.toFixed(0));
    const newAnalysis = analysis.filter((item) => item.materialIndex !== materialIndex.toFixed(0));
  ...
  const handleMaterialChange = (materialIndex: number) => {
    ...
    relatedData.qcParameters.forEach((param: any, idx: number) => {
      const val = Number(getValues(`materials.${materialIndex}.qcResults.${idx}.value`) || 0);
      total -= val;
      const persen = (val / sample) * 100;
      setValue(`materials.${materialIndex}.qcResults.${idx}.persentase`, parseFloat(persen.toFixed(2)));
    });
    setValue(`materials.${materialIndex}.totalBerat`, sample - total);
    setValue(`materials.${materialIndex}.qcKotoran`, total);
    setValue(`materials.${materialIndex}.persentaseKotoran`, parseFloat(((total / sample) * 100).toFixed(2)));
  };
  ...
  const removeItem = (idxToRemove: number) => {
    ...
    const newFields = fields.filter((field: any, idx: number) => field.arrivalItemId !== idxToRemove);

    console.log(idxToRemove, newFields);

    setValue(`materials`, newFields);
  };
  ...
  const onSubmit = async (data: QcSubmissionData) => {
    ...
    data.materials.forEach((material, materialIndex) => {
      formData.append(`materials.${materialIndex}.arrivalItemId`, material.arrivalItemId);
      formData.append(`materials.${materialIndex}.statusQc`, material.statusQc);
      formData.append(`materials.${materialIndex}.qcSample`, material.qcSample.toString());
      formData.append(`materials.${materialIndex}.qcKotoran`, material.qcKotoran.toString());
      formData.append(`materials.${materialIndex}.totalBerat`, material.totalBerat.toString());
      formData.append(`materials.${materialIndex}.pengeringan`, material.pengeringan?.toString() || "");
      console.log("Material:", formData);
      if (material.qcNotes) {
        formData.append(`materials.${materialIndex}.qcNotes`, material.qcNotes);
      }
      if (material.analysis) {
        formData.append(`materials.${materialIndex}.analysis`, material.analysis);
      }

      material.qcResults.forEach((result: any, resultIndex) => {
        formData.append(`materials.${materialIndex}.qcResults.${resultIndex}.parameterId`, result.parameterId);
        formData.append(`materials.${materialIndex}.qcResults.${resultIndex}.value`, result.value);
      });

      if (material.qcPhotos) {
        material.qcPhotos.forEach((photo, photoIndex) => {
          formData.append(`materials.${materialIndex}.qcPhotos.file.${photoIndex}`, photo.file);
        });
      }
    });
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(state.message || `Hasil QC untuk ID ${relatedData.arrival.idKedatangan} berhasil disimpan!`);
      router.push("/qc");
  ...
  return (
    ...
    >
      ...
      <h2 className="text-xl md:text-3xl font-bold">Review Hasil Quality Control</h2>
      <div className="dark:shadow-slate-700 p-8 rounded-xl shadow-lg flex flex-col gap-3">
        <div className="mb-6 flex flex-col md:flex-row gap-2 justify-between items-center">
          <div className="text-md md:text-xl font-semibold w-full flex items-center gap-2">
            <ClipboardList size={24} /> ID Kedatangan: <span className="text-blue-600 font-bold md:text-2xl">{relatedData.arrival.idKedatangan}</span>
          </div>
          <div className="md:text-md flex flex-col md:flex-row w-full items-center gap-4">
            <div className="flex items-center gap-2 w-full">
              <Truck size={20} /> Supplier: <div>{relatedData.arrival.supplier.name}</div>
            </div>
            <div className="flex items-center gap-2 w-full">
        ...
        {fields.map((field: any, materialIndex: number) => {
          ...
          const groupedResults = groupQcResultsByParameter(materialQc.QcResults);
          ...
          return (
            <div key={field.id} className="mb-8 p-6 border border-gray-200 rounded-xl bg-gray-50 dark:bg-gray-900">
                  ...
                  <table className="min-w-full bg-white dark:bg-black border border-gray-200 rounded-lg">
                    <thead className="bg-gray-100 dark:bg-gray-900">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left text-xs font-medium uppercase">Tambahan</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td className="px-4 py-3 text-sm">Berat Uji Sampel</td>
                        <td className="px-4 py-3 text-sm">{materialQc.qcSample || 0}</td>
                        <td className="px-4 py-3 text-sm">-</td>
                        <td className="px-4 py-3 text-sm"></td>
                      </tr>

                      {groupedResults.map((result: any) => (
                        <tr key={result.parameterId} className="border-t border-gray-200">
                          <td className="px-4 py-3 text-sm">{result.parameterName}</td>
                          <td className="px-4 py-3 text-sm">{result.mainValue || 0}</td>
                          <td className="px-4 py-3 text-sm">{materialQc.qcSample ? parseFloat((((result.mainValue || 0) / materialQc.qcSample) * 100).toFixed(2)) : 0} %</td>
                      ...
                      <tr className="border-t border-gray-200">
                        ...
                        <td className="px-4 py-3 text-sm">{materialQc.totalBerat || 0}</td>
                        <td className="px-4 py-3 text-sm">-</td>
                        <td className="px-4 py-3 text-sm"></td>
                      </tr>
                      <tr className="border-t border-gray-200">
                        <td className="px-4 py-3 text-sm">Air & kotoran</td>
                        <td className="px-4 py-3 text-sm">{materialQc.qcKotoran || 0}</td>
                        <td className="px-4 py-3 text-sm">{materialQc.qcSample ? parseFloat((((materialQc.qcKotoran || 0) / materialQc.qcSample) * 100).toFixed(2)) : 0} %</td>
                        <td className="px-4 py-3 text-sm"></td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div className="mb-4">
                <div>
                  <label className="block text-sm font-semibold mb-2">Status QC : {relatedData.qcStatuses.find((status: any) => status.id == materialQc.qcStatusId)?.name || "Belum Di QC"}</label>
                </div>
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm font-semibold mb-2">Catatan QC</label>
        ...
        {relatedData.role == "manager" && (
          <div className="flex gap-4">
            <Button className="bg-blue-400 text-white p-2 rounded-md">Approve</Button>
          </div>
        )}
      </div>
    </form>
