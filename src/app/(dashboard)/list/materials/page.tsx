...
const MaterialListPage = async ({
    ...
    <TableRow key={item.id}>
      ...
      <TableCell>
        <div className="flex items-center gap-2">
          <Link href={`/list/materials/${item.id}`}>
          ...
          {role === "admin" ||
            (role === "superadmin" && (
              <>
                <FormContainer table="material" type="update" data={item} id={item.id} />
  ...
  const query: Prisma.MaterialsWhereInput = {};
  ...
  const [data, count] = await prisma.$transaction([
    ...
    prisma.materials.count({ where: query }),
  ]);
  ...
  return (
    <div className="@container/main flex flex-1 flex-col gap-2 px-4">
      ...
      <div className="flex items-center justify-between py-4">
        <h1 className="hidden md:block text-lg font-semibold">All Materials</h1>
